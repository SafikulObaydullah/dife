...
function CaseStatusLoad() {

    $.ajax({
        url: "/CaseStatus/GetCaseStatusData",
        method: "GET",
        dataType: "json",
...
function CaseStatusDataBind() {
    $("#gridTable").kendoGrid({
        dataSource: CaseStatusList,
        sortable: true,
        toolbar: ["search"],
        search: {
            fields: ["nameE", "nameB"]
...
function Save() {
    ...
    if (validate == true) {
        ...
        $.ajax({
            url: "/CaseStatus/CaseStatusOrSave",
            type: "POST",
            dataType: "json",
            data: o,
            success: function (data) {
...
function Edit(id) {
    ...
    var FilterData = _.filter(CaseStatusList, function (item) { return item.id == id });
