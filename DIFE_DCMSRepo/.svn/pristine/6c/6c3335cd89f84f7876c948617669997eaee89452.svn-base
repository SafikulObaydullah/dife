...
var CaseRespondentOfficeList = [];
var CaseRespondentPersontList = [];
...
function CaseReportBind(id) {
    $.ajax({
        ...
        success: function (data) {
            ...
            CaseRespondentOfficeList = data.caseRespondent; 
            CaseRespondentPersontList = data.caseRepondentPerson; 
            CaseDocumentList = data.caseDocument;
            ShowCaseData();
            ShowCaseRespondent();
            ShowCaseRespondentOffice();
            ShowActivityData();
            ShowAttachments();
            ShowReferenceCase();
        }
...
function ShowCaseRespondent() {
    var respondentHtml = '';
   if (CaseRespondentPersontList.length > 0) {
        respondentHtml += '<tr>' +
            '<th>Category</th>' +
            '<th>Respondent Name</th>' +
            '<th>Desgination</th>' +
            '<th>Address</th>' +
            '<th>Establishment Info</th>' +
            '<th>Type of Respomdent</th>' +
            '</tr>'
      for (var i = 0; i < CaseRespondentPersontList.length; i++) {
            respondentHtml += '<tr>' +
               '<th>' + CaseRespondentPersontList[i].categoryName + '</th>'+
               '<th>' + CaseRespondentPersontList[i].nameOfRespondent + '</th>'+
               '<th>' + CaseRespondentPersontList[i].designation + '</th>'+
               '<th>' + CaseRespondentPersontList[i].address + '</th>'+
               '<th>' + CaseRespondentPersontList[i].respondentAsName + '</th>'+
               '<th>' + CaseRespondentPersontList[i].typeOfEstablishment + '</th>'+
            '</tr>'
        }
    }
    else {
   ...
   $('#caseRespondentPersonTable').append(respondentHtml);
}
function ShowCaseRespondentOffice() {
   var respondentHtml = '';
   if (CaseRespondentOfficeList.length > 0) {
      respondentHtml += '<tr>' +
         '<th>Respondent Office</th>' +
         '<th>Respondent Type</th>' +
         '</tr>'
      for (var i = 0; i < CaseRespondentOfficeList.length; i++) {
         respondentHtml += '<tr>' +
            '<th>' + CaseRespondentOfficeList[i].officeName + '</th>' +
            '<th>' + CaseRespondentOfficeList[i].respondentType + '</th>' +
            '</tr>'
      }
   }
   else {
   ...
   $('#caseRespondentOfficeTable').append(respondentHtml);
}
